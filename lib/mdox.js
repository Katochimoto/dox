...
exports.docGenerate = function(root/*options*/) {
    ...
    files.forEach(function(file, idx) {
        ...
        var tmpJsonSource = path.join(tmpByFileDir, dirSource, baseSource + '.json');
        var tmpMdFileSource = path.join(tmpByFileDir, dirSource, baseSource + '.md');
        var tmpFileSource = path.join(tmpByFileDir, dirSource, baseSource + '.html');
        ...
        mkdirp.sync(path.dirname(tmpJsonSource));
        fs.writeFileSync(tmpJsonSource, JSON.stringify(data), { encoding: 'utf8' });
    ...
    /*
     ...
     var contextTOC = {};
    fileTOC.push({
        title: titleFile || file,
        linkSource: tmpFileSource,
        linkMd: tmpMdFileSource,
