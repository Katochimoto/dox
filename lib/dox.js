...
exports.parseTag = function(str) {
    ...
    var partsString = multiparts.shift();

    var tagTypeMatch = partsString.match(/^@([a-z]+)/i);
    if (!tagTypeMatch) {
        return {};
    }
    ...
    partsString = partsString.replace(/^@[a-z]+/i, '').trim();

    switch (type) {
        // @see http://usejsdoc.org/tags-example.html
        case 'example':
            tag.string = multiparts.join('\n').trim();
            var titleMatch = partsString.match(/^<caption>(.+)<\/caption>$/);
            if (titleMatch) {
        ...
        case 'argument':
        case 'param':
            var typesMatch = partsString.match(/^(\{{1,2}.*?\}{1,2})/);
            if (typesMatch) {
                partsString = partsString.replace(/^\{{1,2}.*?\}{1,2}/, '').trim();
                types = exports.parseTagTypes(typesMatch[1]);

                if (types.types.length) {
                    tag.types = types.types;
                    if (types.nullable) {
                        tag.nullable = true;
                    }

                    if (types.nonNullable) {
            ...
            var nameMatch = partsString.match(/^(\[([^\[\]]+?)\]|[a-z0-9]+)/i);
            if (nameMatch) {
                partsString = partsString.replace(/^\[[^\[\]]+?\]|[a-z0-9]+/i, '').trim();
            ...
            if (partsString) {
                tag.description = partsString;
            }
            break;

        case 'return':
            types = exports.parseTagTypes(parts.shift());
